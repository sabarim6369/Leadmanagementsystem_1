...
const Viewmore = ({ selectedtelecaller, closeModal,databasename}) => {
  ...
  const handleSave = async () => {
    ...
    try {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/telecaller/addfiles`, formData, {
            headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-2xl w-[60%] max-w-3xl shadow-lg relative">
        ...
        <button
          onClick={closeModal}
          className="absolute top-4 right-4 text-gray-700 hover:text-red-600 transition duration-300"
        >
                      ...
                      {selectedtelecaller.files.length > 0 ? (
                        selectedtelecaller.files.map((file) => (
                          <div key={file._id} className="flex items-center justify-between bg-gray-100 p-3 rounded-lg shadow">
                            <span className="text-sm truncate w-64">{file.filename}</span>
                            <div className="flex gap-3">
                              <FiDownload className="text-green-600 cursor-pointer hover:text-green-800" onClick={() => handleDownloadFile(file.fileUrl)} />
